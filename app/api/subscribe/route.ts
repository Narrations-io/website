...
// Source is written to a Notion Select property, where every new value creates
// a new option — allowlist it so callers can't pollute the database schema.
const SOURCES = new Set(["footer", "resources", "site"]);
...
export async function POST(req: Request) {
  ...
  const source =
    typeof body.source === "string" && SOURCES.has(body.source.trim())
      ? body.source.trim()
      : "site";

...
export async function GET(req: Request) {
  // Read-only, called on every calendar month change — generous ceiling.
  if (!rateLimit(`slots:${clientIp(req)}`, 30, 60_000)) {
    return tooMany();
  }
  ...
  if (!res.ok || json.status === "error") {
    ...
    console.error("cal.com slots failed", res.status, json?.error?.message);
    return NextResponse.json(
      { ok: false, error: "Cal.com error" },
      { status: 502 },
    );
  }

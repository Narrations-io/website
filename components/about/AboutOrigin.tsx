...
// /about — origin story, told through four build-stages of the N-mark. Colors mirror
// NMark.tsx's light-tone constants (LIGHT_INK/ACCENT_LIGHT/LIGHT_BG) since these SVGs are
// a partial-construction variant of that same mark, not a new brand element — kept in
// sync manually if NMark.tsx's palette ever changes.
//
// Steps 1–3 are the FOUNDERS' decade; step 4 is the company, registered 2024 (deck §1).
// That split is the point of the section — don't let the two blur back together.
//
...
const STEPS = [
  {
    label: "2016",
    title: "Two founders, one obsession",
    body: "In 2016 the founders started writing for other companies: content, campaigns and the documents behind them, first as freelancers, then inside the digital asset industry. Every project began the same way, by re-explaining what the company had already decided.",
    stage: (uid: string) => (
  ...
  },
  {
    ...
    body: "The work ran through Upwork, where the founders' freelance work reached the top 3% globally across 100+ projects, and through longer contracts. Each added to a private set of notes on what the client had decided: the habit the ledger grew from.",
    stage: (uid: string) => (
      <svg viewBox="0 0 120 120">
  ...
  },
  {
    ...
    body: "From there the founders worked with startups and institutions across the digital asset industry — wallets, exchanges, protocol foundations and payments — and in commercial real estate. The first Narrations clients came from that network.",
    stage: (uid: string) => (
      <svg viewBox="0 0 120 120">
        <polygon points="26,26 70,26 70,40 40,40 40,94 26,94" fill={ACCENT} />
        <polygon points="94,94 50,94 50,80 80,80 80,26 94,26" fill={ACCENT} />
  ...
  },
  {
    ...
    body: "Narrations is now one company: two founders and their AI agents, funded by its own work, with client work under way and the platform in private beta. The rules it keeps for each client are the point.",
    stage: (uid: string) => (
      <svg viewBox="0 0 120 120">
        <defs>
      ...
      <div className="mx-auto max-w-[1200px] px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-ink-900 md:text-[2.75rem]">
            Two founders, ten years each, narrating.
          </h2>
        </div>

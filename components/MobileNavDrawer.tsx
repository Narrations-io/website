...
export default function MobileNavDrawer({
  ...
  // The trigger and drawer are both `md:hidden`: crossing 768px while open
  // (tablet rotation) would hide the drawer but leave the body scroll-locked
  // with no visible way to close it, so close as soon as md is reached.
  useEffect(() => {
    if (!open) return;
      ...
      {open &&
        ...
        createPortal(
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            />
            <div
              className="absolute inset-y-0 right-0 flex w-[82vw] max-w-[360px] flex-col overflow-y-auto bg-green-50 p-6 shadow-pop motion-safe:animate-[navSlideIn_0.25s_cubic-bezier(0.16,1,0.3,1)]"
              role="dialog"
              aria-modal="true"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  aria-label="Close menu"
                  onClick={() => setOpen(false)}
                  className="-mr-2 flex h-11 w-11 items-center justify-center rounded-full text-ink-900 transition hover:bg-paper"
                >

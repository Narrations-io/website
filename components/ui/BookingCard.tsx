          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-2.5 sm:p-4">
            <div className="flex items-center justify-between px-1">
              <button
                ...
                onClick={prevMonth}
                className="flex h-10 w-10 items-center justify-center rounded-lg text-white/40 transition hover:bg-white/[0.06] hover:text-white"
              >
              ...
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                ...
                return (
                  <button
                    ...
                    // w-full + max-w lets the cell shrink with its
                    // minmax(0,1fr) column instead of overflowing it: a fixed
                    // width needs more grid than exists both on phones (~278px
                    // at 390px) and in the md two-column band, where the card
                    // is ~323-448px wide and the month grid gets ~110-180px.
                    className={
                      selected
                        ? "mx-auto flex h-10 w-full max-w-[44px] items-center justify-center rounded-lg bg-green-500 font-semibold text-white sm:h-8 sm:max-w-[36px]"
                        : available
                          ? "mx-auto flex h-10 w-full max-w-[44px] items-center justify-center rounded-lg text-white/75 transition hover:bg-white/[0.08] sm:h-8 sm:max-w-[36px]"
                          : "mx-auto flex h-10 w-full max-w-[44px] items-center justify-center rounded-lg text-white/20 sm:h-8 sm:max-w-[36px]"
                    }
                  >
                    {day}
                  </button>

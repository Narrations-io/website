    ...
    >
      ...
      <div className="relative z-10" style={{ fontSize: "calc(1 * var(--u))" }}>
        ...
        >
          <nav className="mx-auto flex h-[64px] max-w-[1072px] items-center justify-between px-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <MobileNavDrawer theme="light" links={NAV_LINKS} />
            </div>
          </nav>
        </div>
        ...
        <header className="px-6 text-center" style={{ marginTop: "max(40px, calc(6.83 * var(--u)))" }}>
          ...
          <p
            className="nh-anim mx-auto max-w-[760px] text-pretty text-[15px] leading-[1.6] text-[#232323] sm:text-wrap sm:text-[14px]"
            style={{ marginTop: "max(14px, calc(1.02 * var(--u)))", animationDelay: "0.22s" }}
          >
            An editor for everything your company writes. Built by operators
            behind 200+ projects over 10 years.
          </p>
          <div
      ...
      {/* Mobile AND md+ portrait: hill fills the width. Anchored bottom-0
          rather than by --u math, so it's structurally immune to both the fold
          gap and the logo clip that the desktop branch below had to be fixed
          for — there's no width-driven term to run away. Leave it that way.
          Portrait tablets take this branch too (md:landscape flips both hills
          and the .hero-fold height together): the width-driven desktop fold is
          ~560-600px there, which buried the card behind the grass. */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 w-full md:landscape:hidden">
      ...
      {/* md+ landscape: width is max(100vw, N*--u) so it never renders
          narrower than the viewport (always bleeds off both edges — a fixed
          --u-only width would eventually show a hard-cut image boundary on
          wide screens/high zoom). Top offset keeps the crest-tuck look.
          Portrait stays on the bottom-anchored branch above. */}
      <div
        data-qa="hills"
        className="pointer-events-none absolute left-1/2 z-20 hidden -translate-x-1/2 [container-type:inline-size] md:landscape:block"
        style={{
          width: HILL_W,
